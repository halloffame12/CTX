...
/**
 * ctx launcher — @ctx/cli
 *
 * The real `ctx` is a native Rust binary shipped in an optional platform
 * package (@ctx/cli-<platform>-<arch>). npm installs only the matching one.
...
function locateBinary() {
  ...
  const pkg = `@ctx/cli-${platform}-${arch}`;
  ...
  try {
    ...
    const err = new Error(
      `ctx: no binary found for ${platform}/${arch}. ` +
        ...
        `Reinstall with "npm install -g @ctx/cli" so the matching platform package installs.`
    );
    throw err;
  }
}
