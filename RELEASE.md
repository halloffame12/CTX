...
7. **Publish npm** (triggered automatically on `release` → `npm.yml`, or run
   ...
   - publishes `@ctx/cli-linux-x64`, `-arm64`, `-darwin-x64`, `-arm64`,
     `-win32-x64`, `-arm64`, then `@ctx/cli` (all with provenance).
   - post-install job runs `npx @ctx/cli@<v> --version`.
...
10. **Verify installations** (each needs its ecosystem installed):
    ...
    npm install -g @ctx/cli && ctx --version
    npx @ctx/cli --version
